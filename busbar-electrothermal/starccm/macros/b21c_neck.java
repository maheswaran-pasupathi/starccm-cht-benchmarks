// ============================================================================
//  b21c_neck.java
//  B21-C -- current-crowding geometry set, variant C: straight bar with a
//  narrowed neck. Same overall length (0.300m), thickness (0.005m), current
//  (400A), and physics chain as B12's "v1_constant_properties" case (which
//  stands in for B21-A). Width is 0.040m everywhere EXCEPT a 0.040m-long
//  segment centered at mid-length, where it steps down to 0.020m (50%
//  reduction) -- a straight-sided step, no fillet at the step (unlike
//  B21-E's bend fillet).
//
//  Geometry technique: two symmetric SimpleBlockPart "notches" (one on each
//  +Y/-Y side of the neck segment) boolean-subtracted from the main bar --
//  same SubtractPartsOperation pattern as B21-B's hole, just with block
//  tools instead of a cylinder.
//
//  NOTE (found live): a first run at 3000 iterations (matching B21-B/D/E)
//  left the Energy residual at ~0.076 -- roughly 3-4x higher than the other
//  three variants' residual at the same iteration count -- and its heat
//  balance gate FAILED at 5.03% (<2% gate), while current/power gates still
//  passed. The step change in cross-section at the neck is stiffer to
//  converge than a smooth hole or a uniform bend. Fixed by raising the
//  iteration cap to 5500 (not by loosening the gate) -- see
//  docs/discrepancy_log.md for the full write-up.
//
//  Run: starccm+ -new -batch b21c_neck.java
// ============================================================================
import java.io.*;
import java.util.*;
...
public class b21c_neck extends StarMacro {

    static final double L = 0.300, W = 0.040, T = 0.005;
    static final double W_NECK = 0.020;       // ASSUMED -- 50% width reduction, see source_traceability.csv
    static final double L_NECK = 0.040;       // ASSUMED -- neck segment length, centered at mid-span
    ...
    static final String OUT_PATH = "results/processed/b21c_results.csv";
    static final String TABLE_PATH = "results/processed/b21c_field_export.csv";
    Simulation sim; Units m;

    public void execute() {
        sim = getActiveSimulation();
        m = sim.getUnitsManager().getUnits("m");
        sim.println("=== b21c_neck start ===");
        ...
        bar.setPresentationName("bar_b21c_raw");
        bar.rebuildSimpleShapePart();

        double neckStartX = L / 2.0 - L_NECK / 2.0;
        double neckEndX = L / 2.0 + L_NECK / 2.0;
        double notchDepth = (W - W_NECK) / 2.0;  // material removed from each side

        SimpleBlockPart notchTop = f.createNewBlockPart(sim.get(SimulationPartManager.class));
        notchTop.setCoordinateSystem(sim.getCoordinateSystemManager().getLabCoordinateSystem());
        notchTop.setCorners(new DoubleVector(new double[]{neckStartX, W / 2.0 - notchDepth, -0.001}),
                             new DoubleVector(new double[]{neckEndX, W / 2.0 + 0.001, T + 0.001}));
        notchTop.setPresentationName("notch_top");
        notchTop.rebuildSimpleShapePart();

        SimpleBlockPart notchBot = f.createNewBlockPart(sim.get(SimulationPartManager.class));
        notchBot.setCoordinateSystem(sim.getCoordinateSystemManager().getLabCoordinateSystem());
        notchBot.setCorners(new DoubleVector(new double[]{neckStartX, -W / 2.0 - 0.001, -0.001}),
                             new DoubleVector(new double[]{neckEndX, -W / 2.0 + notchDepth, T + 0.001}));
        notchBot.setPresentationName("notch_bot");
        notchBot.rebuildSimpleShapePart();

        List<GeometryPart> subtractInputs = new ArrayList<GeometryPart>();
        subtractInputs.add(bar);
        subtractInputs.add(notchTop);
        subtractInputs.add(notchBot);
        ...
        barFinal.setPresentationName("bar_b21c");
        splitAll(barFinal, 40.0);

        Region r = mkRegion(barFinal, "bar_b21c");
        meshAll(Arrays.asList(barFinal), 0.0020);  // slightly finer than B21-B/A -- resolve the neck step

        Boundary ioutFace = extremeXFace(r, false);
        Boundary vinFace = extremeXFace(r, true);
        vinFace.setPresentationName("Vin_b21c");
        ioutFace.setPresentationName("Iout_b21c");
        ...
        pc.setPresentationName("b21c-neck");
        ...
        maxSteps.setMaximumNumberSteps(5500);
        sim.getSimulationIterator().run(5500);
        ...
        jDotE.setPresentationName("JdotE_b21c");
        jDotE.setFunctionName("JdotE_b21c");
        jDotE.setDefinition(
        ...
        pIntReport.setPresentationName("P_JdotE_b21c");
        ...
        jmx.setPresentationName("Jmax_b21c"); jmx.getParts().setObjects(Arrays.asList(r)); jmx.setFieldFunction(jMagAll);
        ...
        tmx.setPresentationName("Tmax_b21c"); tmx.getParts().setObjects(Arrays.asList(r)); tmx.setFieldFunction(T_ff);
        ...
        tavg.setPresentationName("Tavg_b21c"); tavg.getParts().setObjects(Arrays.asList(r)); tavg.setFieldFunction(T_ff);
        ...
        csv.append(String.format("b21c_neck,%.2f,%.6f,%.6f,%.6f,%.8f,%.10f,%.6f,%.6f,%.6f,%.6f,%.6f,%.3f,%.4f,%.4f,%s%n",
    ...
    void exportField(Region r, FieldFunction jMag, FieldFunction tField) {
        ...
        table.setPresentationName("b21c_field_export");
