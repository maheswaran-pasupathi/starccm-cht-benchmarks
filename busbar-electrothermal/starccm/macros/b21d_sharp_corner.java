// ============================================================================
//  b21d_sharp_corner.java
//  B21-D -- current-crowding geometry set, variant D: an L-shaped bar with a
//  SHARP 90-degree inner corner. Two legs of 0.150m each (0.300m total
//  developed length, matching the straight-bar baseline), same
//  0.040 x 0.005 m cross-section, same 400A current, same physics chain as
//  B12's "v1_constant_properties" case (B21-A).
//
//  Geometry technique: 3D-CAD Sketch (rectangular path with a sharp corner)
//  + SweepMerge of a rectangular cross-section profile along it -- the
//  EXACT proven technique from personal-mentor hubbell_cae Case 3
//  (case3_bent_busbar_current.java), including its sweep-profile-centering
//  fix (profile centered [-W/2, W/2] on the path centerline, not offset
//  [0, W] from one edge -- a real bug found and fixed in that case).
//
//  Run: starccm+ -new -batch b21d_sharp_corner.java
// ============================================================================
import java.io.*;
import java.util.*;
...
public class b21d_sharp_corner extends StarMacro {

    static final double WC = 0.040, TC = 0.005;   // MUST match baseline cross-section
    static final double L1 = 0.150, L2 = 0.150;   // ASSUMED -- sum = 0.300m, matches baseline developed length
    ...
    static final String OUT_PATH = "results/processed/b21d_results.csv";
    static final String TABLE_PATH = "results/processed/b21d_field_export.csv";
    Simulation sim; Units m;

    public void execute() {
        sim = getActiveSimulation();
        m = sim.getUnitsManager().getUnits("m");
        sim.println("=== b21d_sharp_corner start ===");

        SolidModelManager models = sim.get(SolidModelManager.class);
        CadModel cad = models.createSolidModel();
        cad.setPresentationName("b21dCAD");
        ...
        // Profile centered on the path centerline -- [-WC/2, WC/2], NOT
        // [0, WC] -- the fix found live in hubbell_cae Case 7.
        ...
        sweep.setPresentationName("b21dSweep");
        ...
        bar.setPresentationName("bar_b21d");
        splitAll(bar, 40.0);

        Region r = mkRegion(bar, "bar_b21d");
        meshAll(Arrays.asList(bar), 0.0025);

        Boundary vinFace = extremeXFace(r, true);
        Boundary ioutFace = extremeYFace(r, false);
        vinFace.setPresentationName("Vin_b21d");
        ioutFace.setPresentationName("Iout_b21d");
        ...
        pc.setPresentationName("b21d-sharp-corner");
        ...
        jDotE.setPresentationName("JdotE_b21d");
        jDotE.setFunctionName("JdotE_b21d");
        jDotE.setDefinition(
        ...
        pIntReport.setPresentationName("P_JdotE_b21d");
        ...
        jmx.setPresentationName("Jmax_b21d"); jmx.getParts().setObjects(Arrays.asList(r)); jmx.setFieldFunction(jMagAll);
        ...
        tmx.setPresentationName("Tmax_b21d"); tmx.getParts().setObjects(Arrays.asList(r)); tmx.setFieldFunction(T_ff);
        ...
        tavg.setPresentationName("Tavg_b21d"); tavg.getParts().setObjects(Arrays.asList(r)); tavg.setFieldFunction(T_ff);
        ...
        csv.append(String.format("b21d_sharp_corner,%.2f,%.6f,%.6f,%.6f,%.8f,%.10f,%.6f,%.6f,%.6f,%.6f,%.6f,%.3f,%.4f,%.4f,%s%n",
        ...
        sim.println("=== b21d_sharp_corner done ===");
    }

    /** Sharp-cornered path: leg1 along +X from (0,0) to (L1,0), leg2 along +Y from (L1,0) to (L1,L2). */
    private void drawSharpPath(Sketch s) {
    ...
    void exportField(Region r, FieldFunction jMag, FieldFunction tField) {
        ...
        table.setPresentationName("b21d_field_export");
    ...
    /** unique extreme (min or max) area-averaged X face. */
    ...
    /** unique extreme (min or max) area-averaged Y face -- the leg2 end cap. */
